...
export default class CubeNavigationHorizontal extends PureComponent<
  CubeNavigationHorizontalProps,
  CubeNavigationHorizontalState
> {
  ...
  constructor(props: CubeNavigationHorizontalProps) {
    ...
    const { responderCaptureDx, loop, callbackOnSwipe, initialPage } =
      this.props;
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderGrant: () => {
        ...
        this._animatedValue.setOffset({
          x: this.accessibleAnimatedValue.x,
          y: this.accessibleAnimatedValue.y,
        });
      },
      onPanResponderMove: (e, gestureState) => {
        if (loop) {
          if (
            gestureState.dx < 0 &&
            this.accessibleAnimatedValue.x < -1 * this.state.fullWidth
          ) {
            this._animatedValue.setOffset({ x: windowWidth, y: 0 });
          } else if (
            gestureState.dx > 0 &&
            this.accessibleAnimatedValue.x > 0
          ) {
            this._animatedValue.setOffset({
              x: -1 * (this.state.fullWidth + windowWidth),
              y: 0,
            });
          }
        }
    ...
    this.accessibleAnimatedValue = {
      x: pagePositions[this.state.currentPage],
      y: 0,
    };

    this.setAccessibleAnimatedValue =
      this.setAccessibleAnimatedValue.bind(this);
  ...
  computeAnimatedSlideViewStyles(additionalTransforms: any) {
  ...
  _getTransformsFor(index: number) {
    ...
    let loopVariable: any = (variable: number, sign = 1) =>
      variable + Math.sign(sign) * (fullWidth + windowWidth);
  ...
  componentDidMount() {
    this.animatedListenerId = this._animatedValue.addListener(
      this.setAccessibleAnimatedValue
    );
  }
  ...
  renderChildren() {
    const { children, expandView }: any = this.props;
    ...
    return children.map((childItem, index) => {
      ...
      let expandStyle = expandView
        ? this.getComputedExpandedChildViewStyles()
        : this.getComputedNormalChildViewStyles();
